...
# 🔧 Assicura che esista la cartella di backup
...
async def receive_db(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_user.id != ADMIN_CHAT_ID:
        await message.reply_text("❌ Non sei autorizzato a inviare il database.")
        return
    ...
    await message.reply_text(f"✅ Database aggiornato con successo.\nBackup salvato come: {os.path.basename(backup_path)}")